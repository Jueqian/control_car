...
static int terminate_after_junjie = 0;
...
static int frame_send(int tty_fd, const unsigned char *frame, int frame_len)
{
    ...
    result = write(tty_fd, frame, frame_len);    // @taih: 这里有问题，导致终端乱码
...
#define MAX_FRAME_SIZE 13
    ...
    if (frame_send(tty_fd, data_frame, data_frame_len) < 0)     // @taih: 这里有问题，导致终端乱码
    {
...
static int inject_data_frame(int tty_fd, const char *hex_id, const char *hex_data)
{
    ...
    // 这里不要死循环发送，可以多发几帧就行
    while (program_running && !error && terminate_after_junjie > 0)
    {
        ...
        error = send_data_frame(tty_fd, CANUSB_FRAME_STANDARD, binary_id_lsb, binary_id_msb, binary_data, data_len);        // @taih: 这里有问题，导致终端乱码
        terminate_after_junjie--;
    }

    // 关闭这个
    while (program_running && !error && false)
    {
        if (gap_ts.tv_sec || gap_ts.tv_nsec)
            nanosleep(&gap_ts, NULL);
...
/*
sudo ./canusb -d /dev/ttyUSB0 -s 500000 -t -i 601 -j 2f00200001000000 -b 115200

设置速度
sudo ./canusb -d /dev/ttyUSB0 -s 500000 -t -i 601 -j 2B01200088130000 -b 115200
sudo ./canusb -d /dev/ttyUSB0 -s 500000 -t -i 601 -j 2B01200000000000 -b 115200
*/
            ...
            float gap_time_, int can_mode_)
{
    ...
    if (print_traffic_)
        print_traffic++;
    tty_device = tty_device_; // 这里拷贝问题注意
    speed = canusb_int_to_speed(canusb_int_speed_);
    baudrate = baudrate_;
    inject_sleep_gap = gap_time_;
    inject_payload_mode = can_mode_;

    if (tty_device == NULL)
    {

        fprintf(stderr, "Please specify a TTY!\n");
        display_help("junejie");
        return EXIT_FAILURE;
    }
    if (speed == 0)
    {
    ...
    std::cout << "set can success !!" << std::endl;
    return EXIT_SUCCESS;
}
...
{
    terminate_after_junjie = terminate_after_; // 在发送几帧后结束
    inject_data = data_;
    inject_id = ID_;

    if (inject_data == NULL)
    {
        std::cout << "inject_data == NULL" << std::endl;
    ...
    else
    {
        ...
        if (inject_data_frame(tty_fd, inject_id, inject_data) == -1)        // @taih: 这里有问题，导致终端乱码
        {
            return EXIT_FAILURE;
        }
        else
    ...
    std::cout << "can_send_data success : " << data_ << std::endl;
    return EXIT_SUCCESS;
}
